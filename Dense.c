...
static void _SIMD_BackDense(PULSE_Layer * this)
{
	...
	if(this->parent != NULL)
	{
		for(i = 0, wi = 0; i < this->n_outputs; i++, wi += this->n_inputs)
		{
			...
			for(j = 0; j < this->n_inputs - __PULSE_SIMD_N_PER_CHUNK; j += __PULSE_SIMD_N_PER_CHUNK)
			{
				gradients = __PULSE_SIMD_ALLIGNED_LOAD(dense.gradients + wi + j);
				inputs = __PULSE_SIMD_ALLIGNED_LOAD(this->inputs + j);
				gradients = __PULSE_SIMD_MADD(delta, inputs, gradients);
				__PULSE_SIMD_STORE(&dense.gradients[wi + j], gradients);
				weights = __PULSE_SIMD_ALLIGNED_LOAD(dense.weights + wi + j);
				errors = __PULSE_SIMD_ALLIGNED_LOAD(this->parent->errors + j);
				__PULSE_SIMD_STORE(this->parent->errors + j ,__PULSE_SIMD_MADD(weights, delta, errors));
			}
			for(; j < this->n_inputs; j++)
			{
				gradients[wi + j] += cdelta * this->inputs[j];
...
static size_t PULSE_PullDenseTmp(PULSE_Layer * this, PULSE_DataType * FIXES) {
	...
	return this->n_inputs*this->n_outputs + this->n_outputs + this->n_outputs;
}
...
PULSE_Layer PULSE_CreateDenseLayer(PULSE_DenseLayerArgs args, PULSE_DataType *MODEL, PULSE_DataType * IO)
{
	...
	layer.pull_tmp = &PULSE_PullDenseTmp;
