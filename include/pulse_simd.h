...
#define __PULSE_SIMD_STREAM_STORE(X, Y) _Generic((Y), \
		__m128: _mm_stream_ps, \
		__m256: _mm256_stream_ps)(X, Y)
...
#if __PULSE_SIMD_CHUNK_SIZE == 256
...
#if __PULSE_SIMD_CHUNK_SIZE == 128
#define __PULSE_SIMD_DATATYPE __m128
...
static inline __m256 __PULSE_SIMD_X86_ZERO_R_256(__m256 x, int n) {
    return _mm256_blendv_ps(x, _mm256_setzero_ps(), _mm256_castsi256_ps(_mm256_cmpgt_epi32(_mm256_set_epi32(7,6,5,4,3,2,1,0), _mm256_set1_epi32(7 - n))));
};
static inline __m128 __PULSE_SIMD_X86_ZERO_R_128(__m128 x, int n) {
    return _mm_blendv_ps(x, _mm_setzero_ps(), _mm_castsi128_ps(_mm_cmpgt_epi32(_mm_set_epi32(3,2,1,0), _mm_set1_epi32(3 - n))));
};
static inline __m128 __PULSE_SIMD_X86_REDUCE_ADD_128(__m128 x) {
    __m128 odd = _mm_movehdup_ps(x);
    __m128 sum = _mm_add_ps(x, odd);
    return _mm_add_ss(_mm_movehl_ps(odd, sum), sum);
};
static inline __m128 __PULSE_SIMD_X86_REDUCE_ADD_256(__m256 x) {
    __m128 tmp = _mm_add_ps(_mm256_castps256_ps128(x), _mm256_extractf128_ps(x, 1));
    return __PULSE_SIMD_X86_REDUCE_ADD_128(tmp);
};
