...
int main()
{
    ...
    struct pulse_layer_s* model = pulse_create_model(2, pulse_dense_layer(DTYPE, INPUT_DIMENSION, 4, ReLU, LR), pulse_dense_layer(DTYPE, 4, OUTPUT_DIMENSION, Sigmoid, LR));

    clock_t t1 = clock();
    pulse_train(model, 10000, SAMPLES, 1, MSE, X, Y);
    clock_t t2 = clock();
